...
async def 도움(ctx, *arg):
    ...
    embed.add_field(name = "정보 [이름]", value = "대상의 정보를 봅니다. (유효하지 않은 대상이면 본인의 정보가 나옵니다.)", inline = False)
    embed.add_field(name = "랭킹 [스트릭 / 골드 / 최장 / 문제]", value = "전체 사용자들의 랭킹을 봅니다.", inline = False)
    embed.add_field(name = "서버랭킹 [스트릭 / 골드 / 최장 / 문제]", value = "서버 사용자들의 랭킹을 봅니다.", inline = False)
    ...
    embed.add_field(name = "문의 [내용]", value = "봇의 관리자에게 문의합니다.", inline = False)
    await ctx.send(embed = embed)
